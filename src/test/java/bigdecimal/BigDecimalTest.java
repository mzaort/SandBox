...
public class BigDecimalTest {
	...
	public void testPrecison() {
		BigDecimal n1 = new BigDecimal("-0.1234");
		System.out.println(n1.precision());
		System.out.println(n1.scale());
		System.out.println(n1.unscaledValue());
		BigDecimal n2 = n1.setScale(10);
		System.out.println(n2.precision());
		System.out.println(n2.unscaledValue());
		System.out.println(n2.scale());
		double d = 1.311313131231312312312312312;
	...
	public void testBigDecimal() {
		...
		// when creating a BigDecimal, using a string, not a double. Because
		// double can not be stored precisely
	...
	public void testCompareAndReminder() {
		System.out.println(BigDecimal.valueOf(0.0).compareTo(BigDecimal.valueOf(-0.0)));
		System.out.println(BigDecimal.valueOf(-34.1).remainder(BigDecimal.ONE));
		System.out.println(new BigDecimal("1.e2000").doubleValue() == Double.POSITIVE_INFINITY);
		...
		System.out.println(
				new BigDecimal(-1.25).setScale(1, BigDecimal.ROUND_HALF_UP).stripTrailingZeros().toPlainString());
	...
	public void testScientificNotation() {
		/*
		 * The toString method for a BigDecimal behaves differently based on the
		 * scale and precision. (Thanks to @RudyVelthuis for pointing this out.)
		 * If scale == 0, the integer is just printed out, as-is. If scale < 0,
		 * E-Notation is always used (e.g. 5 scale -1 produces "5E+1") If scale
		 * >= 0 and precision - scale -1 >= -6 a plain decimal number is
		 * produced (e.g. 10000000 scale 1 produces "1000000.0") Otherwise,
		 * E-notation is used, e.g. 10 scale 8 produces "1.0E-7" since precision
		 * - scale -1 equals is less than -6.
		 */
		BigDecimal bd = null;
